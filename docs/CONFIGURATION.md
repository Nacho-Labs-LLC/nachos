...
Messaging platform connections. Each channel is optional and disabled by
default.
...
**`[runtime.state.identity]`, `[runtime.state.memory]`,
`[runtime.state.user_profile]`, `[runtime.state.bootstrap]`**
...
> **Per-spawn override:** Pass `sandboxMode: "host"` or `sandboxMode: "full"` in
> the `sessions_spawn` tool call to override the global sandbox mode for a
> specific subagent.
...
Array of declarative job definitions synced to the SQLite job registry on
startup.
...
1. **Schema types** (`schema.ts`) — TypeScript interfaces defining the config
   shape
2. **CONFIG_SHAPE** (`validation.ts`) — Structural allowlist for unknown-key
   detection
