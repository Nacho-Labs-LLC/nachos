 ...
 */
interface PythonExecutorParameters extends ToolParameters {
 ...
 */
export class PythonExecutor extends ToolService {
  readonly toolId = 'code_runner_python';
  readonly name = 'Python Code Runner';
  readonly description = 'Execute Python code in a sandboxed environment';
  ...
  private async createTempScript(code: string, workdir: string): Promise<string> {
    ...
    const scriptPath = path.join(workdir, `script-${randomUUID()}.py`);
