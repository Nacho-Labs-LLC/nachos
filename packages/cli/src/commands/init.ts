...
export async function initCommand(options: InitCommandOptions): Promise<void> {
  ...
  try {
    ...
    writeFileSync(join(cwd, 'policies', `${initOptions.securityMode}.yaml`), policyContent, 'utf-8');
...
${options.enableWebchat ? `[channels.webchat]
enabled = true
port = 8080` : ''}
...
function generateEnv(options: InitOptions): string {
  const lines = ['# Nachos Environment Variables', '# DO NOT commit this file to version control', ''];
