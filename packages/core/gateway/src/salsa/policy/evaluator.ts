...
export class PolicyEvaluator {
  ...
  evaluate(request: SecurityRequest): SecurityResult {
    ...
    return {
      ...
      reason:
        this.defaultEffect === 'deny' ? 'No policy rule matched - default deny applied' : undefined,
      evaluationTimeMs,
    };
  }
  ...
  private matchesCriteria(request: SecurityRequest, match: PolicyMatch): boolean {
    ...
    // Check resource ID
    if (match.resourceId) {
      const resourceIds = Array.isArray(match.resourceId) ? match.resourceId : [match.resourceId];
   ...
   */
  private matchesConditions(request: SecurityRequest, conditions: PolicyCondition[]): boolean {
   ...
   */
  private matchesCondition(request: SecurityRequest, condition: PolicyCondition): boolean {
