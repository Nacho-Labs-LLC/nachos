...
describe.skipIf(!ollamaAvailable)('Ollama E2E Integration (llama3.2)', () => {
  ...
  it(
    'TC-O1: basic chat returns well-formed response',
    async () => {
  ...
  it(
    'TC-O2: max_tokens is enforced (not ignored like max_completion_tokens)',
    async () => {
      const result = await adapter.send(
        {
          messages: [
  ...
  it(
    'TC-O3: system messages are handled correctly',
    async () => {
      const result = await adapter.send(
        {
          messages: [
            {
              role: 'system',
              content: 'You are a pirate. Always start your response with "Arrr!"',
            },
  ...
  it(
    'TC-O4: tool calling returns structured tool_calls',
    async () => {
      const result = await adapter.send(
        {
          messages: [
  ...
  it(
    'TC-O5: streaming returns delta and done chunks',
    async () => {
  ...
  it(
    'TC-O6: streaming with tool use emits tool_call chunks',
    async () => {
      const chunks: LLMStreamChunkType[] = [];
      const result = await adapter.stream(
        {
  ...
  it(
    'TC-O7: wrong model name returns a clear error',
    async () => {
      try {
        await adapter.send(
  ...
  it(
    'TC-O8: multi-turn conversation works correctly',
    async () => {
      const result = await adapter.send(
        {
          messages: [
